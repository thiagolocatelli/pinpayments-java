...
import com.github.thiagolocatelli.pinpayments.deserializer.DateDeserializer;
...
public abstract class APIResource extends APIObject {
	...
	protected static String singleClassURL(Class<?> clazz) {
		return String.format("%s/%s/%s", PinPayments.getApiBase(), PinPayments.VERSION, className(clazz));
	}
	...
	static Map<String, String> getHeaders(String apiKey) {
		...
		headers.put("User-Agent",
				String.format("PinPayments/v1 JavaBindings/%s", PinPayments.VERSION));

		if (apiKey == null) {
			apiKey = PinPayments.apiKey;
		}
		...
		propertyMap.put("bindings.version", PinPayments.VERSION);
		propertyMap.put("lang", "Java");
		propertyMap.put("publisher", "PinPayments");
		headers.put("X-PinPayment-Client-User-Agent", GSON.toJson(propertyMap));
		if (PinPayments.apiVersion != null) {
			headers.put("PinPayments-Version", PinPayments.apiVersion);
		}
		return headers;
	}
	...
	protected static <T> T request(APIResource.RequestMethod method,
			...
			APIException, ResourceNotFoundException, InvalidResourceException {
	...
	protected static <T> T _request(APIResource.RequestMethod method,
			...
			InvalidRequestException, APIConnectionException,
			APIException, ResourceNotFoundException, InvalidResourceException {
		if ((PinPayments.apiKey == null || PinPayments.apiKey.length() == 0)
				&& (apiKey == null || apiKey.length() == 0)) {
			throw new AuthenticationException("api_key_missing",
					"No API key provided. (HINT: set your API key using 'PinPayments.apiKey = <API-KEY>'. "
							+ "You can generate API keys from the PinPayments web interface. ");
		}
	...
	private static void handleAPIError(String rBody, int rCode)
		...
		switch (rCode) {
		case 400:
			throw new InvalidRequestException(error.error, error.errorDescription);
		case 404:
			throw new ResourceNotFoundException(error.error, error.errorDescription);
		case 401:
			throw new AuthenticationException(error.error, error.errorDescription);
		case 422:
			throw new InvalidResourceException(error.error, error.errorDescription, error.messages);
		default:
